...
#    cp -f /fhem/FHEM/00_OPENgate.pm ${FHEM_DIR}/FHEM/
...
echo "fhem ALL=(ALL) NOPASSWD:SETENV: /bin/bash *" >> /etc/sudoers.d/fhem-docker
echo "fhem ALL=(ALL) NOPASSWD:SETENV: /bin/echo *" >> /etc/sudoers.d/fhem-docker
...
if [ ! -s ${FHEM_DIR}/.ssh/id_ed25519 ]; then
  ...
  echo "$i. Copy SSH Ed25519 client certificate to 'host' ..."
  cat ${FHEM_DIR}/.ssh/id_ed25519.pub >> /opt/host/home/deos/.ssh/authorized_keys
  (( i++ ))
fi
...
# SSH ID Transfer to host
echo "$i. Transfer PUB-SSH to 'host' ..."
[ -z "$(cat /opt/host/home/deos/.ssh/authorized_keys | grep -e '.*fhem@fhem-docker.*')" ] && cat ${FHEM_DIR}/.ssh/id_ed25519.pub >> /opt/host/home/deos/.ssh/authorized_keys
...
# Added +nocookie because ;; Warning: Client COOKIE mismatch
if [ -z "$(dig +short +nocookie -t a gateway.docker.internal.)" ]; then
...
if [ -z "$(dig +short +nocookie -t a host.docker.internal.)" ]; then
